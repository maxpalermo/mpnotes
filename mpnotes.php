...
class MpNotes extends Module
{
    public function __construct()
    {
        ...
        if (file_exists(dirname(__FILE__) . '/config/config.yml')) {
            $this->configLoader = new YamlFileLoader(
                $this->get('kernel')->getContainer()->get(
                    'config_loader_resolver'
                ),
                new FileLocator(dirname(__FILE__) . '/config')
            );
        }
    ...
    public function hookDisplayAdminEndContent($params)
    {
        ...
        $notes = $this->getNotes('customer', $id_customer);
