async function importEmbroideryList() {
    const confirm = await swalConfirm("Importare i messaggi dei ricami?");
    if (!confirm) {
        return;
    }
    ...
    try {
        ...
        progressData.start();
        progressData.setMessage("Ricerca Messaggi ricami");
        ...
        if (data.list.length > 0) {
            ...
            progressData.start();
            console.log("Importazione messaggi ricami", data.list.length);
            doImport(data.list, "embroidery", progressData, id_note_type);
        } else {
            Swal.fire({
                icon: "info",
                title: "Nessun messaggio ricami trovato",
