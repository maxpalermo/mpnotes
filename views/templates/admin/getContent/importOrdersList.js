async function importOrdersList() {
    const confirm = await swalConfirm("Importare i messaggi ordini?");
    if (!confirm) {
        return;
    }
    ...
    try {
        ...
        progressData.start();
        progressData.setMessage("Ricerca Messaggi ordini");
        ...
        if (data.list.length > 0) {
            ...
            progressData.start();
            console.log("Importazione messaggi ordini", data.list.length);
            doImport(data.list, "order", progressData, id_note_type);
        } else {
            Swal.fire({
                icon: "info",
                title: "Nessun messaggio ordine trovato",
