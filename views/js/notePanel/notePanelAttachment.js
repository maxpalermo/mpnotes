...
async function notePanelAttachmentLoaded() {
    ...
    document.addEventListener("click", async function (e) {
        if (e.target.closest(".btn-delete-attachment")) {
            ...
            swalConfirm("Sei sicuro di voler eliminare questo allegato?", "Questa azione non può essere annullata").then(async (result) => {
                if (result.isConfirmed) {
                    try {
                        const formData = new FormData();
                        formData.append("ajax", 1);
                        formData.append("action", "deleteAttachment");
                        formData.append("id_attachment", attachmentId);
                        ...
                        if (data.success) {
                            swalSuccess(data.message || "Allegato eliminato con successo").then(() => {
                                // Remove attachment from carousel
                            ...
                            swalError(data.message || "Si è verificato un errore durante l'eliminazione dell'allegato");
                        }
                    } catch (error) {
                        swalError("Si è verificato un errore durante la comunicazione con il server");
                        console.error(error);
